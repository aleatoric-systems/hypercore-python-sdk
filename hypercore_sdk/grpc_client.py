...
class GrpcClient:
    ...
    def get_mid_price(self, *, coin: str = "BTC") -> dict[str, Any]:
        request = bridge_pb2.MidPriceRequest(coin=coin)  # type: ignore[attr-defined]
        with self._channel() as channel:
    ...
    def get_block_number(self) -> dict[str, Any]:
        request = bridge_pb2.BlockNumberRequest()  # type: ignore[attr-defined]
        with self._channel() as channel:
            stub = bridge_pb2_grpc.PriceServiceStub(channel)
            start = time.perf_counter()
    ...
    def stream_mids(
        ...
        *,
        ...
        request = bridge_pb2.StreamMidsRequest(  # type: ignore[attr-defined]
            coin=coin,
    ...
    def stream_liquidations(
        ...
        *,
        ...
        request = bridge_pb2.StreamLiquidationsRequest(  # type: ignore[attr-defined]
            coin=coin,
            heartbeat_s=heartbeat_s,
        )
        output: list[dict[str, Any]] = []
        with self._channel() as channel:
            stub = bridge_pb2_grpc.PriceServiceStub(channel)
            stream = stub.StreamLiquidations(
                request,
                timeout=max(self.cfg.timeout_s, float(heartbeat_s) * max_messages + 2),
                metadata=self._metadata(),
            )
